...
def test_review_required_without_old_candidate_is_not_single_review():
...
def test_one_old_one_new_non_complex_review_stays_single():
    old_item = item("OLD", "H6", "H6多少钱？")
    new_item = item("NEW", "H6", "H6 PHEV多少钱？")
    grouped = group_review_required(
        run_result(
            [
                diff(
                    ...
                    old=old_item,
                    new=new_item,
                    reason=ReviewReason.TRIM_CONFLICT,
                ),
            ]
        )
    )
